...
import eu.europeana.metis.ldaggregation.harvesting.ldes.ActivityHandler;
import eu.europeana.metis.ldaggregation.harvesting.ldes.demo.syncdb.TimestampTracker;
 ...
 * 
 */
public class EdmRecordsActivityHandler implements ActivityHandler {
	...
	public EdmRecordsActivityHandler(TimestampTracker timestampTracker, MetadataHarvester seeAlsoHarvester)
			throws Exception {
...
//    seeAlsoHarvester.deleteRecord(manifestStatus.getResourceId());
//     seeAlsoHarvester.save(manifestStatus.getResourceId(), );

	  
	  log(activity.getTypeOfActivity() + " on " + activity.getObject().getURI());
			switch (activity.getTypeOfActivity()) {
			case Add:
			case Create:
			case Update:
				timestampTracker.setObjectTimestamp(currentCrawlStreamUri, activity.getObject().getURI(),
						activity.getTimestamp());
       seeAlsoHarvester.save(activity.getObject().getURI(), activity.getObjectNamedGraph());
				
				break;
			case Delete:
			case Remove:
				timestampTracker.setObjectTimestamp(currentCrawlStreamUri, activity.getObject().getURI(),
						activity.getTimestamp(), true);
				break;
			case Move:
				String targetResource = activity.getTarget().getURI();
				timestampTracker.setObjectTimestamp(currentCrawlStreamUri, activity.getObject().getURI(),
						activity.getTimestamp(), true);
				timestampTracker.setObjectTimestamp(currentCrawlStreamUri, targetResource,
						activity.getTimestamp());
			case Refresh:
